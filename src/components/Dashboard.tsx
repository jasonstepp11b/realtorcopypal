...
export default function Dashboard() {
  ...
  const { user, userProfile, session, refreshSession } = useSupabaseAuth();
  ...
  useEffect(() => {
    if (user && session) {
      console.log("Dashboard component: Fetching initial data");
      fetchListings();
      fetchRecentProjects();
    }
  }, [user, session, fetchListings, fetchRecentProjects]);

  // Refresh data when the document becomes visible
  useEffect(() => {
    const handleVisibilityChange = async () => {
      if (document.visibilityState === "visible" && user && session) {
        console.log(
          "Dashboard component: Visibility changed to visible, refreshing data"
        );
  ...
  }, [user, session, refreshSession, fetchListings, fetchRecentProjects]);
